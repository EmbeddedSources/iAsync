...
static void logErrnoGlobalVariable( const char* attributeName_ )
{
    switch ( errno )
    {
        case EEXIST:
        ...
        case ENOATTR:
        {
            ...
            NSLog( @"!!!ERROR!!! %s attribute does not exist", attributeName_ );
            break;
        }
        case ENOTSUP:
        {
...
-(void)addSkipBackupAttribute
{
    ...
    const char* attributeName_ = "com.apple.MobileBackup";
    int result_ = setxattr( [ self fileSystemRepresentation ], attributeName_, &b, 1, 0, 0 );

    if ( result_ == -1 )
    {
        logErrnoGlobalVariable( attributeName_ );
    }
}
